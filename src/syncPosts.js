...
const PUPPETEER_USER_AGENT = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/117.0.0.0 Safari/537.36';
...
async function fetchPostContent(link) {
  ...
  try {
    ...
    throw new Error(`Puppeteer failed to scrape content from ${link}: ${err.message}`);
  } finally {
    if (browser) {
      await browser.close();
    }
...
async function syncNewPosts() {
  if (!RSS_PAGE_URL) {
    console.error('[POST SYNC] Error: RSS_PAGE_URL is not defined in the .env file.');
    return;
  }
  ...
  try {
    ...
    newPosts.reverse();
    console.log(`[POST SYNC] Found ${newPosts.length} new post(s). Fetching content in chronological order...`);
    ...
    for (let i = 0; i < newPosts.length; i++) {
      ...
      try {
        ...
        console.log(`[POST SYNC] Successfully saved post ID ${nextId}: "${title}"`);
        
        nextId++;

        if (i < newPosts.length - 1) {
          // MODIFIED: Use the configured interval for the log message
          console.log(`[POST SYNC] Waiting ${RSS_CHECK_INTERVAL_SECONDS} seconds to respect rate limit...`);
          // MODIFIED: Use the configured interval (converted to milliseconds) for the delay
          await sleep(RSS_CHECK_INTERVAL_SECONDS * 1000);
        }

      } catch (err) {
        console.error(`[POST SYNC] Failed to process post "${post.title}". Error:`, err);
      }
    }
